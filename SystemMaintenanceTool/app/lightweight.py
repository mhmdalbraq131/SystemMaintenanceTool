...
def run_native(command, timeout=900):
    ...
    p=subprocess.run(command,capture_output=True,text=True,encoding="utf-8",errors="replace",timeout=timeout,creationflags=flags)
    return p.returncode,((p.stdout or "")+"\n"+(p.stderr or "")).strip()
class App(tk.Tk):
    def __init__(self):
        super().__init__(); self.title("أداة صيانة النظام"); self.geometry("1240x760"); self.minsize(1000,640); self.configure(bg=BG)
        self.protocol("WM_DELETE_WINDOW",self.destroy); self._build_style(); self._build_layout(); self.show("dashboard"); self.after(500,self.refresh_dashboard); self.after(3500,self._live_refresh)
    ...
    def _build_layout(self):
        ...
        nav=tk.Frame(body,bg=PANEL,width=225); nav.pack(side="right",fill="y"); nav.pack_propagate(False); tk.Label(nav,text="الإدارة والصيانة",bg=PANEL,fg=MUTED,font=("Segoe UI",10,"bold")).pack(fill="x",padx=18,pady=(22,10))
        items=[("dashboard","لوحة المعلومات"),("system","معلومات النظام"),("processes","العمليات"),("storage","التخزين"),("network","الشبكة"),("security","الأمان"),("maintenance","الصيانة والإصلاح"),("services","خدمات Windows"),("startup","بدء التشغيل"),("commands","مركز الأوامر"),("reports","التقارير")]
        self.buttons={}
    ...
    def tree(self,parent,cols,heads):
        t=tk.Frame(parent,bg=BG); t.pack(fill="both",expand=True,padx=24,pady=6); tree=ttk.Treeview(t,columns=cols,show="headings"); [tree.heading(c,text=h) for c,h in zip(cols,heads)]; [tree.column(c,anchor="center",width=150) for c in cols]; tree.pack(side="left",fill="both",expand=True); sb=ttk.Scrollbar(t,orient="vertical",command=tree.yview); sb.pack(side="right",fill="y"); tree.configure(yscrollcommand=sb.set); return tree
    def _live_refresh(self):
        self.refresh_dashboard(); self.after(3500,self._live_refresh)
    def refresh_dashboard(self):
        try:
            cpu=psutil.cpu_percent(None); ram=psutil.virtual_memory().percent; disk=max((psutil.disk_usage(p.mountpoint).percent for p in psutil.disk_partitions(all=False) if os.path.exists(p.mountpoint)),default=0); self.dc.config(text=f"{cpu:.0f}%"); self.dr.config(text=f"{ram:.0f}%"); self.dd.config(text=f"{disk:.0f}%"); self.dp.config(text=str(len(psutil.pids()))); self.du.config(text=self.uptime()); self.da.config(text="نعم" if is_admin() else "لا"); worst=max(cpu,ram,disk); self.health.config(text="الحالة: حرجة" if worst>=90 else "الحالة: تحتاج انتباه" if worst>=75 else "الحالة: جيدة")
    ...
    def refresh_network(self):
        ...
        for n,addrs in psutil.net_if_addrs().items(): lines.append(n+":"); lines.extend("  "+a.address for a in addrs)
        self.replace(self.network,"\n".join(lines))
    def refresh_security(self):
        lines=[f"صلاحيات المسؤول: {'نعم' if is_admin() else 'لا'}"]
        if wmi:
            try:
                c=wmi.WMI(); lines += [f"مكافح الفيروسات: {x.displayName} | الحالة: {x.productState}" for x in c.query("SELECT displayName, productState FROM AntiVirusProduct")]
    ...
    def _async_custom(self,raw):
        def work():
            try: code,out=run_native(["cmd","/c",raw],900); self.after(0,lambda:self.replace(self.cmd_output,f"Exit code: {code}\n\n{out}"))
    ...
    def health_check(self):
        ...
        def work():
            try:
                cpu=psutil.cpu_percent(interval=1); ram=psutil.virtual_memory().percent; disk=max((psutil.disk_usage(p.mountpoint).percent for p in psutil.disk_partitions(all=False) if os.path.exists(p.mountpoint)),default=0); worst=max(cpu,ram,disk); msg="الحالة: حرجة" if worst>=90 else "الحالة: تحتاج انتباه" if worst>=75 else "الحالة: جيدة"; self.after(0,lambda:self.health.config(text=f"{msg} — CPU {cpu:.0f}% | RAM {ram:.0f}% | Disk {disk:.0f}%")) ; self.after(0,lambda:self.status.config(text="اكتمل الفحص"))
            except Exception as e: self.after(0,lambda:self.status.config(text=f"فشل الفحص: {e}"))
        threading.Thread(target=work,daemon=True).start()
    def clean_temp(self):
        ...
        if temp and os.path.isdir(temp):
            for root,dirs,files in os.walk(temp,topdown=False):
                for n in files:
                    try: os.remove(os.path.join(root,n)); deleted+=1
                    except OSError: pass
                for n in dirs:
