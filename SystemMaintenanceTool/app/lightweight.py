...
Runtime dependencies: psutil + optional Windows WMI/pywin32 already used by
the project. Tkinter is part of the standard Windows Python installation.
"""
...
class App(tk.Tk):
    ...
    def show(self, key):
        ...
        for k, b in self.buttons.items():
            b.configure(bg=ACCENT if k == key else PANEL)
        self.status.configure(text=self.buttons[key].cget("text"))

    def card(self, parent, title, col, row):
        c = tk.Frame(parent, bg=PANEL, highlightthickness=1,
                     highlightbackground=PANEL2)
        c.grid(row=row, column=col, sticky="nsew", padx=6, pady=6)
        tk.Label(c, text=title, bg=PANEL, fg=MUTED,
                 font=("Segoe UI", 10)).pack(fill="x", padx=14, pady=(12, 2))
        v = tk.Label(c, text="—", bg=PANEL, fg=TEXT,
                     font=("Segoe UI", 20, "bold"))
        v.pack(fill="x", padx=14, pady=(0, 12))
        return v

    def _make_dashboard(self):
        f = self.page("dashboard", "لوحة المعلومات")
        for i in range(3):
        ...
        box.grid(row=2, column=0, columnspan=3, sticky="nsew", padx=6, pady=8)
        self.health = tk.Label(box, text="جاري القراءة...", bg=PANEL, fg="#93c5fd",
    ...
    def _make_system(self):
        f = self.page("system", "معلومات النظام")
    ...
    def _make_processes(self):
        f = self.page("processes", "العمليات")
    ...
    def _make_maintenance(self):
        f = self.page("maintenance", "الصيانة والإصلاح")
        actions = [
        ...
        tk.Label(f, text=("تشغيل كمسؤول: نعم" if is_admin()
    ...
    def _make_services(self):
        f = self.page("services", "خدمات Windows")
        self.services = self.textbox(f, font=("Consolas", 9))
        tk.Button(f, text="عرض الخدمات", command=self.refresh_services,
                  bg=ACCENT, fg="white", relief="flat", padx=18, pady=8).pack(
                  anchor="e", padx=24, pady=5)

    def _make_startup(self):
        f = self.page("startup", "بدء التشغيل")
        self.startup = self.textbox(f, font=("Consolas", 9))
        tk.Button(f, text="فحص بدء التشغيل", command=self.refresh_startup,
                  bg=ACCENT, fg="white", relief="flat", padx=18, pady=8).pack(
                  anchor="e", padx=24, pady=5)

    def _make_commands(self):
        f = self.page("commands", "مركز الأوامر")
        ...
        tk.Button(f, text="تنفيذ الأمر", command=self.run_custom_command,
                  bg=ACCENT, fg="white", relief="flat", padx=18, pady=8).pack(
                  anchor="e", padx=24, pady=5)
        self.cmd_output = self.textbox(f, font=("Consolas", 9))

    def _make_reports(self):
        f = self.page("reports", "التقارير")
        self.report = self.textbox(f, font=("Consolas", 9))
    ...
    def textbox(self, parent, font=("Segoe UI", 10)):
        t = tk.Text(parent, bg=PANEL, fg=TEXT, insertbackground="white",
                    relief="flat", wrap="word", font=font)
        t.pack(fill="both", expand=True, padx=24, pady=6)
        return t
    ...
    def tree(self, parent, cols, heads):
        ...
        for c, h in zip(cols, heads):
            t.heading(c, text=h)
            t.column(c, anchor="center", width=150)
        t.pack(fill="both", expand=True, padx=24, pady=6)
        return t
    ...
    def refresh_dashboard(self):
        try:
            ...
            for p in psutil.disk_partitions(all=False):
                try:
                    disks.append(psutil.disk_usage(p.mountpoint).percent)
                except OSError:
                    pass
            disk = max(disks, default=0)
            ...
            self.status.config(text=f"خطأ: {e}")
    ...
    def refresh_system(self):
        ...
        lines = [
            ...
            f"المعالج: {cpu} أنوية فعلية / {logical} منطقية",
    ...
    def refresh_processes(self):
        for x in self.proc.get_children():
            self.proc.delete(x)
        rows = []
        for p in psutil.process_iter(["pid", "name", "cpu_percent", "memory_percent"]):
            try:
                rows.append((p.info["pid"], p.info["name"] or "",
                             p.info["cpu_percent"] or 0, p.info["memory_percent"] or 0))
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                continue
        for r in sorted(rows, key=lambda x: x[2], reverse=True)[:150]:
            self.proc.insert("", "end", values=(r[0], r[1], f"{r[2]:.1f}", f"{r[3]:.1f}"))
    ...
    def refresh_storage(self):
        ...
        for p in psutil.disk_partitions(all=False):
            try:
                ...
                out.append(f"{p.device}  {u.total/1024**3:.1f} GB | "
                           f"متاح {u.free/1024**3:.1f} GB | مستخدم {u.percent:.1f}%")
            except OSError:
                pass
    ...
    def refresh_network(self):
        ...
        lines = [f"إرسال: {c.bytes_sent/1024**2:.1f} MB",
                 f"استقبال: {c.bytes_recv/1024**2:.1f} MB", ""]
        for name, addrs in psutil.net_if_addrs().items():
            lines.append(name + ":")
            lines.extend("  " + a.address for a in addrs)
    ...
    def refresh_security(self):
        ...
        if wmi:
            try:
                c = wmi.WMI()
                for av in c.query("SELECT displayName, productState FROM AntiVirusProduct"):
                    lines.append(f"مكافح الفيروسات: {av.displayName} | الحالة: {av.productState}")
            except Exception as e:
                lines.append(f"تعذر قراءة WMI للأمان: {e}")
        else:
            lines.append("WMI غير متاح؛ تم استخدام الموارد الأساسية فقط.")
        self.replace(self.security, "\n".join(lines))

    def refresh_services(self):
        self._async_command(["sc", "query", "type=", "service", "state=", "all"],
                            self.services, 20)
    ...
    def _command_done(self, target, text, show_window):
        ...
        if target is not None:
            ...
            t = self.textbox(win, font=("Consolas", 9))
            self.replace(t, text)
    ...
    def clean_temp(self):
        ...
        if temp and os.path.isdir(temp):
            for root, dirs, files in os.walk(temp, topdown=False):
                for name in files:
                    try:
                        os.remove(os.path.join(root, name)); deleted += 1
                    except OSError:
                        pass
                for name in dirs:
                    try:
    ...
    def refresh_report(self):
        ...
        for p in psutil.disk_partitions(all=False):
            try:
                u = psutil.disk_usage(p.mountpoint)
                data["disks"].append({"device": p.device, "total_gb": round(u.total/1024**3, 2),
                                      "free_gb": round(u.free/1024**3, 2), "used_percent": u.percent})
            except OSError:
                pass
    ...
    @staticmethod
    def uptime():
        sec = max(0, int(datetime.now().timestamp() - psutil.boot_time()))
